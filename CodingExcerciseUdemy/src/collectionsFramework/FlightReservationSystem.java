...
public class FlightReservationSystem {
	     ...
	     */   
	     ...
	     if (confirmedList.size() == 10 && waitingList.size() == 3) {
	            return false;
	        }
	     else {
	            ...
	            if ((confirmedList.size() == CONFIRMEDLIST_LIMIT) && (waitingList.size() < 3)) {

	                waitingList.add(passenger);
	                System.out.println("test of waitingList: " + waitingList);
	            }
	            
	            if (confirmedList.size() < CONFIRMEDLIST_LIMIT) {

	                confirmedList.add(passenger);
	                System.out.println("test of confirmedList" + confirmedList);
